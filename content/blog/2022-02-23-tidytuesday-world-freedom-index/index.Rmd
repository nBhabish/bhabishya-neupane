...
freedom %>% 
  filter(year == 2020) %>% 
  pivot_longer(cols = civil_liberties:political_rights, names_to = "metrics") %>% 
  mutate(metrics = metrics %>% str_replace_all("_", " ") %>% str_to_title()) %>% 
  mutate(region_name = region_name %>% fct_reorder(value)) %>% 
  count(region_name, metrics, value) %>% 
  ggplot(aes(value, n, fill = region_name))+
  geom_col()+
  facet_grid(region_name~metrics)+
  theme_light()+
  theme(text = element_text(family = "BenchNine"))+
  scale_fill_paletteer_d("nationalparkcolors::Arches")+
  labs(fill = "", x = "", y = "# of countries")
```
